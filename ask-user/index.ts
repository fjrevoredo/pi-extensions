...
export default function askUser(pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		promptGuidelines: [
			...
			"Every explicit ask_user option must include value, label, description, and responseType: use responseType=select for fixed choices or responseType=freeText with both freeTextMode and freeTextPlaceholder for text branches.",
			"Never include freeTextMode or freeTextPlaceholder on an ask_user responseType=select option, and do not omit responseType.",
			...
			"For bulk workflows, add an explicit fixed ask_user option such as archive-all; never infer multiple selections from a free-text response.",
