...
The strongest evidence for this rule came from typechecking `permission-gate/core.ts` for the first time: `event.toolName !== "bash"` does **not** narrow pi's `ToolCallEvent` union, because `CustomToolCallEvent.toolName` is a plain `string`. An extension-registered tool named `bash` therefore reaches the gate with an arbitrary payload, which is why `normalizeCommand` takes `unknown` rather than `string` and why that coercion is load-bearing rather than defensive noise. Nothing in a JSDoc typedef would have surfaced that, in the one extension that most needs it surfaced.
...
**U4 — SHOULD: use theme semantic colors; never hard-code ANSI.** Where no semantic color fits, document the substitution at the call site — and **substitute an orthogonal attribute, not a neighbouring color.** This rule previously held up `context-footer` borrowing `mdHeading` for orange as the model, and that example was wrong: a theme may alias `mdHeading` to the same color as `success` — the `matrix` theme aliases both to `#00FF41` — so the orange context tier rendered byte-identically to the healthy green tier, and crossing 200k tokens read as the meter *recovering*. A semantic color name is a promise about role, not about appearance, and two names may resolve to one color. `context-footer` now uses bold `warning`: bold composes with whatever the palette does, so it cannot collide. **A fake theme in a test must resolve names through a palette**, aliases included; one that echoes the requested name reports two tiers as distinct while the terminal paints them the same, which is how this survived (T3).
...
The rule exists because this was once false in the worst possible place: the `ask-user` and `permission-gate` trees — 2,211 lines, including the most intricate code in the repo — were covered by no `tsconfig.json` at all and were never typechecked, and `context-footer` was the only extension that was.
...
**Its quality gate went red, and the findings were fixed rather than suppressed.** Two ratings on new code failed — Security, on `npm ci` running lifecycle scripts, and Reliability, on a genuinely quadratic regex in `permission-gate`'s catalogue, a zero-width character class in `ask-user`, and an object-literal parameter default in `advisor`. Three of the four were worth fixing on their own terms and one was a false positive fixed by not writing the construct at all. That is the useful way to read this analyser: **it is a second opinion, not a gate, and the response to a finding is to evaluate it, not to satisfy it.** Where a finding is wrong, the fix is to make the code say what it means more plainly, because a construct two independent analysers misread is one the next human reader will misread too — `ask-user`'s `visibleWidth` had already needed a four-line comment and a `biome-ignore` to defend a character class, and replacing it with a numeric range test needed neither. Suppression comments were deliberately not used anywhere. Nothing in this paragraph promotes SonarCloud to a gate: it still asserts no rule in this document, it holds no branch protection, and a finding it raises that this standard contradicts is resolved in favour of this standard.
