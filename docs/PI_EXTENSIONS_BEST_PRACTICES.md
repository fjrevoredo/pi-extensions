...
- **An OS matrix.** `advisor/test/path-access.test.ts` calls `symlink()`, which needs privileges on Windows; a macOS runner costs 10× to re-test what the developer's own machine covers on every commit. Useful side effect of the split: local runs exercise macOS openrsync, CI exercises GNU rsync 3.x, and the L7 exclusion list is now **observed** to behave identically under both — not merely expected to (§14).
